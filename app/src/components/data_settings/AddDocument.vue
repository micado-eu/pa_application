...
export default {
  ...
  methods: {
    ...
    createShell () {
      this.int_doc_shell = {
        id: -1, issuer: null, translations: [], pictures: [], icon: '', model: null, validable: false
      }
      this.languages.forEach((l) => {
        //       console.log(l)
        this.int_doc_shell.translations.push({
          id: -1, lang: l.lang, document: '', description: '', translationDate: null
        })
      })
    },
    addHotspot (picture) {
      const selected_picture = this.uploaded_images.filter((pic) => {
    ...
    mergeDoc (doc) {
      ...
      doc.translations.forEach((doc) => {
        ...
        for (let i = 0; i < this.int_doc_shell.translations.length; i++) {
    ...
    removePicture (image) {
      const idx = this.uploaded_images.findIndex((an_image) => an_image === image)
      this.uploaded_images.splice(idx, 1)
      console.log(this.uploaded_images)
      const doc_idx = this.int_doc_shell.pictures.findIndex((an_image) => an_image.image === image)
  ...
  created () {
    ...
    if (this.thedocumenttypeid != null) {
      const filtered_type = this.document_types.filter((the_doc_type) => the_doc_type.id == this.thedocumenttypeid)[0]
